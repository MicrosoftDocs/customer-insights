...
- Transaction History: Historical transactions log with semantic data schema below
...
1. Define the percentile of your active customers you consider to be **High-value customer**.
    ...
    - **Percent of top active customers**: If you have a specific percentile for a high-value customer, choose this option and set the percentile. For example, you can use this option to define high-value customers as top 20% of future paying customers.
...
1. Add optional customer data or select **Next** and go to [Set update schedule](#set-update-schedule).
...
- **Training model performance**: A, B, or C are possible grades. This grade indicates the performance of the prediction and can help you make the decision to use the results stored in the output entity.
...
- **Value of customers by percentile**: Customers are grouped into low-value and high-value and shown in a chart. Hover over the bars in the histogram to see the number of customers in each group and the average CLV of that group. Optionally, [create segments of customers](segments.md) based on their CLV predictions.
