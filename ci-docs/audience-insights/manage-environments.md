...
### Enable data share with Dataverse from your own data storage (preview)
Enabling data sharing with Microsoft Dataverse from your own Azure Data Lake Storage as CI data storage will need additional configuration steps.
   1. Create two security groups on your Azure subscription - one Reader security group and one Contributor security group and make Microsoft Dataverse service as the owner for both security groups.
   2. Manage the Access Control List (ACL) on the CustomerInsights container in your storage account via these security groups - Add Microsoft Dataverse service and any Dataverse based business applications like Dynamics365 Marketing to the Reader security group to **read-only** permissions, and add **only** Customers Insights application to the Contributor security group to grant both **read and write** permissions to write profiles and insights.
   
#### Prerequisites
To execute the PowerShell scripts you need to have the below three modules imported. 
...
2. Import three modules.
    ...
    2. Repeat this for 'Install-Module -Name Az.Resources' and 'Install-Module -Name Az.Storage'.

#### Configuration steps
1. Go to this [GitHub location](https://github.com/trin-msft/byol) and download the two PowerShell scripts you need to run.
    1. CreateSecurityGroups.ps1
       1. You need tenant admin permissions to run this PowerShell script. 
       2. This PowerShell script creates two security groups on your Azure subscription - one for the Reader group and another for the Contributor group and will make Microsoft Dataverse service as the owner for both these security groups.
       3. Execute this PowerShell script in Windows PowerShell by providing the Azure subscription id containing your Azure Data Lake Storage. Open the PowerShell script in a notepad or a textpad to review additional information and the logic implemented.
       4. Copy both the security group id values generated by this script into a clipboard or a notepad to be used in the second PowerShell script mentioned below.
       5. Note: Security group creation can be disabled on your tenant. In that case, a manual setup would be needed and your Azure AD admin would have to enable security group creation. https://docs.microsoft.com/en-us/azure/active-directory/enterprise-users/groups-self-service-management
