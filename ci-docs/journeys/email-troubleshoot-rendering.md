...
Rendering emails exactly as designed is critical for protecting the company’s brand. However, email clients vary widely in how they interpret standard HTML. To address these differences, the email designer applies a post‑processing step that modifies the HTML to work around known limitations in various email clients. As a result, the final email HTML may be more complex than expected. While this process ensures consistent rendering across most email clients, some limitations cannot be fully resolved.

This article documents known limitations and recommended mitigations. Because email clients evolve over time, issues may be fixed or new issues may be introduced anytime. We will keep updating this page as we learn about these changes.

## All email clients

### Dark mode changes email colors unexpectedly

Most email clients automatically adjust email colors when the recipient's device is set to dark mode. This behavior varies by client and cannot be controlled from the email editor. Any color changes that occur in dark mode are expected behavior of the email client.
...
### Custom content blocks are not supported out of the box

Custom content blocks are not supported by default. When custom HTML code is used in a content block, the customer is responsible for ensuring that the HTML renders correctly across email clients.

#### Suggested solution

At this time there is no known solution for this issue.
...
Outlook (Classic) does not natively support CSS background images on sections and columns.
...
- **Rounded corner buttons**: Buttons with rounded corners placed on a section or column with a background image will have their border radius removed automatically, because VML-based buttons and VML backgrounds are not compatible.
...
Outlook (Classic) renders line height larger than web-based clients, resulting in more vertical space between lines of text than intended. Additionally, numeric line-height values (for example, `line-height: 1.5`) are not supported.
...
Outlook (Classic) doesn't support selectively breaking only long words; it supports either breaking all words or none. Long words or URLs that do not break can cause the width of a section to expand beyond its intended size.
...
In the designer, set the image width to `100%` instead of `auto`. This prevents Outlook (Classic) from expanding the table when the image dimensions cannot be determined in advance.
...
Underline color on linked text does not match text color
...
1.	Select the text.
2.	Apply the underline style.
3.	Add a hyperlink to the selected text.
4.	Set the font color on the selected text.
Applying the color as the last step ensures that both the text and the underline inherit the same color.
Custom bullet styles do not render
...
When a section or column uses a background image and also contains dynamic text (for example, {{FirstName}}), the pre-calculated height used to render the VML background image cannot account for the actual height of the dynamic content. This can cause the background image to be clipped or misaligned.
Suggested solution
Currently there is no known solution for this issue.
...
When a custom font is used that Outlook (Classic) does not support, it may not fall back to the specified fallback font.
Suggested solution
...
Outlook mobile
Table layouts are not responsive
Note: This issue occurs only when you modify or replace the HTML generated by the email designer with your own custom HTML, overriding the code intended by the email designer to handle this correctly 
...
Suggested solution
Use table-based HTML layouts instead of <div>-based layouts. This reduces the need for Outlook conditional comments. As a result, Outlook conditional comments for tables are not added when spacing is set to 0.
T-online
Rendering issues affect email layout and appearance
The T-online email client has specific rendering limitations that can affect email layout and appearance. For details, see the T-online issues page in the internal team wiki.
Suggested solution
At this time there is no known solution for this issue.
Orange mobile
Div-based layouts are not supported
Note: This issue occurs only when you modify or replace the HTML generated by the email designer with your own custom HTML, overriding the code intended by the email designer to handle this correctly 

The Orange mobile email client supports only HTML3 and table-based layouts. Emails that use <div>-based layouts will not render correctly.
Suggested solution
At this time there is no known solution for this issue.
