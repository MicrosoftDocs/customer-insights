...
### Checking current license ownership
- As a tenant admin, you can see what licenses are on your tenant by going to the [Microsoft Admin Center](https://admin.microsoft.com) then going to **Billing** > **Your Products**. Because Customer Insights licenses aren't seat-based (that is, you don't pay per user) you won't see them listed under **Licenses**. Only licenses where you pay per user and assign users show up under **Licenses**.
...
- The new entitlements do not apply to subscribers of the old, standalone models. For example, if you own the old standalone Dynamics 365 Marketing license you do not gain access to install Customer Insights - Data. You must buy the new Dynamics 365 Customer Insights base license to gain the new entitlements. 
...
- All quota counts at the tenant level across different environment types (sandbox, development, production) because the database capacity and services cost Microsoft the same amount of money regardless of the type of environment or application lifecycle development cycle in which it's being used. All quota usage is summed at the tenant level across all environment types.
...
- The installation management page allows you to install two times more than you're entitled to accommodate migration scenarios. If you need to install either application more than four times, you can buy an additional instance of the base license to get four more application installation entitlements per app.
