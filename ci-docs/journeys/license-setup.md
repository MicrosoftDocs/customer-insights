...
-   Official published pricing and license details including the downloadable Power Platform and Dynamics 365 license guide can be found on our (https://dynamics.microsoft.com/ai/customer-insights/pricing/)[marketing website pricing page]. Please review the license guide for detialed entitlements. 
-   As a tenant admin you can see what licenses are on your tenant by going to the [Microsoft Admin Center](https://admin.microsoft.com) > **Billing** > **Your Products**. Because the Customer Insights licenses are not seat based, that is you do not pay per user, you will not see them listed under **Licenses**. Only licenses where you pay per user and assign users show up under **Licenses**.
-   If you purchased before September 2023 you probably own one or both of the standalone licenses, Dynamics 365 Marketing (Standalone) or Dynamics 365 Customer Insights (Standalone). The new entitlements given with currently available Dynamics 365 Customer Insights license are not grandfathered to the old standalone licenses. You can add the new, Dynamics 365 Customer Insights license to your tenant at any time in your contract cycle to get the new entitlements. Depending on your purchase channel, you can remove the standalone licenses on your renewal date.
-   As of September 2023 when we launched the new licenses and entitlements, licenses are no longer associated with individual application installations as they were prior for the Marketing standalone license model. The applications for Customer Insights - Journeys (formerly called Marketing) and Customer Insights - Data are the same for the old licenses and the new license. **DO NOT UNINSTALL AND REINSTALL applications when you change license types**.
...
-	All quota counts at the tenant level across different environment types (sandbox, development, production) because the database capacity and services cost Microsoft the same amount of money regardless of the type of environment or application lifecycle development cycle in which it is being used. All quota usage is summed at the tenant level across all environment types.
