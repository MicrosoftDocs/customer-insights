...
- **Audience**: Define [how to handle duplicate records](#how-to-handle-duplicate-records) by selecting the [matching rule](real-time-marketing-matching-rules.md). Configure if the form submission can create new records or update existing records. If *Update matched contact with submitted data* and *Create a new contact if there was no match to an existing one?* are set to **No**, the form submission does not update or create any record. The form submission can be linked to an existing record identified by the matching rule.
