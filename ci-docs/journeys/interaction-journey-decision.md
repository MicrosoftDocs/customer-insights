...
1. Make sure the web tracking script comes from the same Customer Insights - Journeys organization that generates the tracking links pointing to your website. Web tracking doesn't work if you copy the script from one organization (for example, a UAT environment) and test it with tracking links generated by a different organization (for example, a production environment).
