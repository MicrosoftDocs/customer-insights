...
> - If multiple people are using the same web browser, web tracking will track their interactions as one person.
...
Additionally, continue reading the next section if you have Content Security Policy (CSP) turned on for your website.

## Conditions needed for web tracking to work if you use CSP
...
When CSP is turned on for your website, then you will have to whitelist the following domains: `script-src`, `connect-src` and `frame-ancestors: https://*.mkt.dynamics.com/ https://*.azureedge.net`. If you don't whitelist them, then scripts to load Customer Insights Journeys Forms will be prevented and forms will not be loaded.
