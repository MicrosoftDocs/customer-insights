...
| Azure Content Delivery Network (CDN) | Production | Disable Azure Content Delivery Network (CDN) for Customer Insights - Journey. |
...
| Double Opt-in | Preview| [Enable double opt-in functionality for real-time journeys. Forms created or modified after enabling this feature will link to a compliance profile and inherit its double opt-in status.](real-time-marketing-double-opt-in.md) |
