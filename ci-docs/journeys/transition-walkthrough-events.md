...
Creating events in real-time journeys is mostly the same as in outbound marketing. However, new features in real-time journeys significantly enhance and improve the overall experience.

To create a new real-time event, you can select **+ Real-time event** in the top navigation bar. You can now see whether the event is created using the outbound marketing or the **Real-time Journeys Marketing module** in the event record:
...
> [!NOTE]
> The system displays a warning if you create outbound marketing events scheduled to start or end after outbound marketing is removed.
...
- Waitlist is now a status of a registration and works for both events and sessions simultaneously. For more information, see [Preview: Set up and manage an event waitlist](set-up-and-manage-waitlist.md).
...
The event schedule should also be taken into consideration when crafting the transition schedule. The key decision point is the registration start and end date. If the registration period ends before the planned outbound marketing removal date, the event can still be created as an outbound marketing event even if the event takes place in the future. 
...
The data structure for real-time journeys events is the same as outbound marketing for now, but as we introduce changes and enhancements to the system, we might implement certain features differently.
...
Below are some relevant blogs and workarounds:

- Cascaded fields where options provided in a field depend on a value in a previous field that isn't supported yet. For more information, see the blog [Implementing cascading dropdown fields in real-time journeys forms](https://community.dynamics.com/blogs/post/?postid=ff86d88f-d892-ef11-ac21-6045bdd7e1ae).
...
If you're not using the out-of-the-box event record creation form, the way to distinguish an outbound marketing record from a real-time journeys record is by setting the **Marketing module** attribute on the Event table (**msevtmgt_sourcesystem**).
...
Existing event forms that were customized for Outbound marketing events won't work for Real-time marketing events. Crucial tabs might be hidden on a customized form. For example, the "Website and Forms" tab is needed for Real-time events to operate as the registration form and session level settings are set on that tab.
