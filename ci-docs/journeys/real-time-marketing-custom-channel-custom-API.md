...
> [!IMPORTANT]
> Custom channels enforce a maximum execution timeout of 20 seconds. If your plugin doesn't complete within this limit, the custom channel request times out. In such cases, the system considers the attempt failed and initiates another attempt to run the plugin. This can lead to discrepancies between expected behavior and actual outcomes, as your plugin might still be running while the custom channel has timed out.
