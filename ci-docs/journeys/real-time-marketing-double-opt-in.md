...
1. You can select whether the double opt-in email is sent with every form submission or whether it's sent only to newly created customers. The condition to determine if the customer is considered new is based on the presence of the contact point consent for the specific email address. If there's no contact point consent for the customer's email address linked to the selected compliance profile, the customer is considered a new one.
