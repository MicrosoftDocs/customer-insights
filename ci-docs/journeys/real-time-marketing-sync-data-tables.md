...
> 2. Adding tables to the segment builder increases Dataverse database storage consumption because additional data must be stored and maintained in your environment. Only add tables when required, and remove any tables that aren't in use. After enabling new tables, monitor your Dataverse storage consumption.
