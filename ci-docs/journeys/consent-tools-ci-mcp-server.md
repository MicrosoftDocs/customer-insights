---
title: Use consent tools in the Customer Insights MCP server
description: Learn how AI agents use the Customer Insights MCP server to check consent and generate unsubscribe links for compliant outbound messaging.
ms.date: 05/27/2026
ms.topic: conceptual
author: PetrJantac
---
...
- Integration with Dynamics 365 Sales, Customer Insights, or Contact Center workflows that need centralized consent checks
...
**Typical use case:** An AI agent preparing to send a commercial message calls `check_consent` to verify the recipient has opted in for the relevant purpose and channel.
...
Generate a personalized unsubscribe link that you can embed directly into outbound messages, ensuring a compliant and user-friendly opt-out experience.

Each link is unique to the contact point and message context, providing full traceability and auditability.
...
2. Call `get_unsubscribe_link` to get a personalized opt-out link.
3. Compose and send the message with the link embedded.
...
2. **Registry layer** - Automatically discovers available tool providers at runtime using reflection-based auto-discovery
3. **Tool layer** - MCP wrappers that validate inputs and delegate to services
4. **Service layer** - Business logic that calls the Backend Consent Service
5. **Data layer** - Dataverse-based storage for consent records

The MCP server definition lives in the base solution (CxpClient), which ships to every Dynamics 365 organization. Consent tools are added via the Consent solution as an extension. The same layered model is used for future tool providers from Customer Insights - Data and Customer Insights - Journeys.
...
Before using the consent tools, you'll need:

- A Dynamics 365 Customer Insights - Journeys environment, or a Dynamics 365 environment with Shared Consent deployed (for example, D365 Sales with Shared Consent)
...
- [Consent management overview](https://learn.microsoft.com/en-us/dynamics365/customer-insights/journeys/real-time-marketing-compliance-settings) - Learn how consent works in Customer Insights - Journeys
- [Extend and customize consent](https://learn.microsoft.com/en-us/dynamics365/customer-insights/journeys/developer/extend-consent) - Developer guide for consent extensibility
