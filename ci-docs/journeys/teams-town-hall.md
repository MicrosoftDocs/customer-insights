...
> [!NOTE]
> To use Microsoft Teams as an online event provider, you must have a [Microsoft 365 license](/office365/servicedescriptions/teams-service-description) that allows you access to the Teams service. If you don't have the correct license, you won't see the Teams Town Hall in Customer Insights - Journeys.
...
Select Teams Town hall as your streaming provider. Once you save your changes the meeting URL for Teams Town Hall gets generated. This URL can then be used to send to your attendees through emails created in Customer Insights-Journeys.

### Town Hall settings

The default Town Hall settings are configured to provide the best attendee and presenter experience. However, you can easily tweak these settings from your Customer Insights - Journeys event. To change the default settings, select **Open settings**. This opens a Teams Town Hall meeting options pop up and allows you to configure the Town Hall experience. Learn more: [How to set up Town Hall meeting options](https://support.microsoft.com/office/schedule-a-town-hall-in-microsoft-teams-d493b5cc-9f61-4dac-8027-d837dafb7a4c).
...
Currently, only organizers can publish the Town Hall Event through Customer Insights-Journeys and cancel the event. Co-organizers, when assigned, are able to edit the meeting options.
...
In scenarios where event record changes aren’t propagated to Teams, the Customer Insights - Journeys app displays a warning to any user who isn’t the Teams meeting owner. If Teams meeting owner opens the event record, they see a **Sync with Teams** button in the ribbon. Selecting the **Sync with Teams** button syncs the changes made to the event by any non-owner users.
...
When the event is set up, you can go ahead and publish the event in Customer Insights - Journeys. Once the event is live, the Town Hall Event is published and calendar invites are automatically generated for organizers and sent to co-organizers and speakers directly from Teams. 
...
Town Hall events don't require users to register for the event so no check-ins are being associated with registrations. The attendance report of attendees who joined the event will be available after the event is over, but it won't be associated with event registrations. The way Teams Town Hall map attendance is via email, therefore you'll see the check-in table populated in Customer Insights - Journeys with attendance data associated with the email address, but not with event registration. 
