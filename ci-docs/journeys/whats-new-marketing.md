...
- Allow editing of default journey time zone in real-time marketing only organizations
    - Organizations that use real-time marketing only can now edit the default time zone used when creating journeys. The default time zone can either be a single time zone across the organization for consistency or it can be set to use the time zone of the user who creates the journey for convenience. To set the default journey time zone, go to Settings and open “Journey Settings” in the “Customer engagement” section.
