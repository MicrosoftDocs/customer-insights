...
> [!IMPORTANT]
> When a Customer Insights journey attempts to send a push notification to an expired device registration token, the system automatically cleans up the expired token.
