...
Suppose you need to include a short blurb about your company (what it does, annual revenue, awards, etc.) or some legal boiler plate text in certain emails. Creating a content block with this content will allow this content to be inserted quickly in any email. Content blocks can also be used for standardizing how certain information is presented. For example, you may create a “product info” content block with a standard layout and styles for showing product name, description, image, link to details, etc. Other email authors can reuse this content block to easily achieve a professional and consistent look for their emails (and saving them time in authoring such content). 

## How are content blocks different from email templates? 

Content block is a **fragment of content** that optionally can also include layout. A content block usually meant to be used as is in emails providing a quick and easy way for content to be reused easily. An email can contain multiple content blocks. Content blocks can be inserted and removed as needed. The same content block can be used in multiple emails. Updating a content block may or may not update emails that include it depending on whether they are "static" or "dynamic" content block (more on this below)). 

 

Email templates, on the other hand, are structured layouts for an **entire email** with placeholder content that is expected to be updated. Only one template can be used to create an email. Updating a template does not update existing emails. Finally, templates may include content blocks but the resvese is not possible.

## Element vs Layout content blocks

There are two types of content blocks based on what they contain:
...
> [!NOTE]
> **Dynamic content blocks** are currently in preview. To enable them, go to **Settings** > **Feature switches** area and set "Dynamic content blocks (preview)" feature switch (in the **Email editor** group) to ON.

There are two types of content blocks based on what happens when their content is updated: 
...
1. Commonly used content such as boiler plate text (legal text, terms and conditions, intro, or closing text) can be saved as an element content block and then quickly inserted into emails. This saves time and ensures that the correct content is used each time. Make it a dynamic content block if it is critical that all emails always use the most current content (e.g., Copyright year or business hours)
...
- **Ready to send** – Content blocks have two states:
    - **On** (default) – The content block is in the *Ready to send* state. It is available for insertion into emails but can't be edited until you switch it back to *Draft*.
    - **Off** – The content block is in the *Draft* state. It can be edited but isn't available for insertion into emails.
...
- **Layout** – Indicates the content block contains layout (section type). If this label is absent, the content block is an element with no layout.
...
When finalizing updates to the content block, you can update the existing content block or save it as a new content block. Saving as a new content block does not change the existing content block. 


## Deleting a content block 

Content blocks can only be deleted from **Assets** > **Content blocks** area. 
