---
title: Baseline (pre-March 2026) permissions for out-of-the-box roles
description: Out-of-the-box role permissions from before March 2026 are preserved here as a historical reference. Start here, then apply each release's changes to stay in sync.
ms.date: 07/28/2026
ms.topic: article
author: vinayd
ms.author: alfergus
...
# Baseline (pre-March 2026) permissions for out-of-the-box roles

>[!NOTE]
> This section doesn't reflect the current roles or their privileges. It preserves the last published version of this article, which documented 11 roles and their privileges across five actions. Use it as a historical reference to validate custom roles against the complete privilege set *prior to the adoption of the changelist format*.

To keep your custom roles in sync, first align them with the documented baseline in this article. Then, apply the permission changes introduced in each release, up to your current release, (detailed in [Permissions for out-of-the-box roles](role-permissions.md)) to bring your roles up to date. Afterward, review the [Permissions for out-of-the-box roles](role-permissions.md) article after every release to identify new changes that must be applied to remain in sync.
