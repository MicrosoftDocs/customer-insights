...
description: Learn how to create, integrate, and finalize a custom trigger to start a journey from a customer action on your site or app.
ms.date: 08/04/2026
ms.topic: how-to
author: cmenesatti-m
ms.author: alfergus
...
This step is performed in the **Triggers** section of Customer Insights - Journeys. To create the custom trigger, select **+New trigger** from the top ribbon. On the following screen, give the trigger a name and select that you want to trigger an action **When a customer interacts with a website/app**.
...
- **Special Attributes**
    ...
    **contactpoint_email** is a special attribute that's present in a custom trigger that is tied to a Customer Insights - Data profile. This attribute is used as a fallback in case the full Customer Insights - Data profile isn't available. Developers should include the customer's email in this attribute to ensure the customer can be reached by email even if their full profile information isn't available.

    **contactpoint_phone** is a special attribute that's present in a custom trigger that is tied to a Customer Insights - Data profile. This attribute is used as a fallback in case the full Customer Insights - Data profile isn't available. Developers should include the customer's phone number in this attribute to ensure the customer can be reached by text messages even if their full profile information isn't available.
...
:::image type="content" source="media/real-time-marketing-move-ingestion.png" alt-text="Screenshot of the option to download the generated code snippet for a custom trigger." lightbox="media/real-time-marketing-move-ingestion.png":::

> [!NOTE]
> To directly access the code snippet, developers need access to the Dynamics 365 Customer Insights - Journeys app.
...
The code snippet contains instructions detailing how to integrate the trigger code on external systems using JavaScript.
...
> [!TIP]
> When you look at the list of triggers, custom triggers have a plain lightning bolt icon :::image type="icon" source="media/real-time-marketing-custom.png":::. Out-of-the-box triggers have a lightning bolt with a suitcase icon :::image type="icon" source="media/real-time-marketing-oob.png":::.
