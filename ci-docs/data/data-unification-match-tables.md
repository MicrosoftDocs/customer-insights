...
description: Matching rules connect records across tables into single customer profiles. Discover how to choose a primary table, add conditions, and handle exceptions.
ms.date: 09/09/2026
ms.topic: how-to
...
In this step, you define the match order and rules for cross-table matching. This step requires at least two tables. When the system matches records, it concatenates them into a single record with all the fields from each table. The system considers alternate rows (nonwinner rows from the deduplication step) when matching. If a row matches an alternate row in a table, the record matches the winner row.

> [!NOTE]
> Once you create your match conditions and select **Next**, you can't remove a selected table or column. If needed, select **Back** to review the selected tables and columns before continuing.
...
> [!IMPORTANT]
> The first table is called the primary table, which serves as the basis for your unified profiles. The system adds your other selected tables to this table.
>
...
1. To treat every record in the table as a unique customer regardless of whether a match is found, select **Include all records**. The unified profile includes any records in this table that don't match to records in any other table. Records that don't have a match are called singletons.
...
Match rules specify the logic for matching a specific pair of tables. A rule consists of one or more conditions.
...
1. In the **Add rule** pane, configure the conditions for the rule.
   ...
   :::image type="content" source="media/m3_add_rule.png" alt-text="Screenshot of the Add rule pane with condition options.":::

   - **Select Table/Field (first row)**: Choose a table and a column that's likely unique to a customer. For example, choose a phone number or email address. Avoid matching by activity-type columns. For example, a purchase ID likely doesn't match other record types.
   ...
   - **Precision**: Set the level of precision for this condition. [Precision is used for exact match and fuzzy matching](data-unification-best-practices.md), and determines how close two strings need to be to be considered a match.
     - **Basic**: Choose from *Low (30%)*, *Medium (60%)*, *High (80%)*, and *Exact (100%)*. Select **Exact** to only match records that match 100 percent.
     - **Custom**: Set a percentage that records need to match. The system only matches records that pass this threshold.
   - **Name**: Name for the rule.

1. To match tables only if columns meet multiple conditions, select **Add** > **Add condition** to add more conditions to a match rule. Conditions are connected with a logical AND operator and execute only if all conditions are met.
...
1. Select **Add rule** on the table where you want to add rules.

1. Follow the steps in [Define rules for match pairs](#define-rules-for-match-pairs).

> [!NOTE]
> The order of rules matters. The matching algorithm tries to match a given customer record based on your first rule and continues to the second rule only if it doesn't find any matches with the first rule.

## Advanced options

### Add exceptions to a rule

In most cases, table matching leads to unique customer profiles with consolidated data. To address rare cases of false positives and false negatives, define exceptions for a match rule. The system applies exceptions after processing the match rules and avoids matching all records that fulfill the exception criteria.

For example, if your match rule combines last name, city, and date of birth, the system identifies twins with the same last name who live in the same town as the same profile. You can specify an exception that doesn't match the profiles if the first name in the tables you combine aren't the same.
...
Specify conditions that override the default match logic. The following options are available:

|Option  |Description |Example  |
|---------|---------|---------|
|Always match     | Defines values for the primary keys that always match.         |  Always match the row with primary key *12345* to the row with primary key *54321*.       |
...
1. Select **Custom**.
   ...
   :::image type="content" source="media/m3_match_custom.png" alt-text="Screenshot of the Custom button.":::
...
1. Open the downloaded template file and fill in the details. The template contains fields to specify the table and the table primary key values to use in the custom match. Table names are case sensitive. For example, if you want primary key *12345* from the *Sales* table to always match with primary key *34567* from the *Contact* table, fill in the template:
    - Table1: Sales
    - Table1Key: 12345
    - Table2: Contact
    - Table2Key: 34567
