...
description: Customer Insights - Data APIs let you build custom apps on your unified customer data. Learn how to enable API access, register an app, and start making calls.
ms.date: 08/31/2026
ms.topic: how-to
...
Dynamics 365 Customer Insights - Data provides APIs to build your own applications based on your data in Customer Insights. You can find details about these APIs in the [API reference](https://developer.ci.ai.dynamics.com/api-details#api=CustomerInsights), which includes information about operations, parameters, and responses.
...
1. If API access to the environment isn't set up, select **Enable**.

   When you enable the APIs, you create a primary and secondary subscription key for your environment that you use in the API requests. To regenerate the keys, select **Regenerate primary** or **Regenerate secondary** on the **APIs** tab.
...
1. Search for and select an API operation and select **Try it**.
   ...
   :::image type="content" source="media/try-api.png" alt-text="Screenshot of how to test the APIs.":::

1. In the side pane, set the value in the **Authorization** dropdown menu to **implicit**. The `Authorization` header gets added with a bearer token. Your subscription key automatically populates.
...
Create a new [app registration](/graph/auth-register-app-v2) to use the APIs in an Azure application by using delegated permissions.
...
Create an app registration that doesn't need user interaction and can run on a server.
...
Get started using the client libraries available for the Customer Insights - Data APIs. You can find all library source code and sample applications on a [GitHub repo](https://github.com/microsoft/Dynamics365-CustomerInsights-Client-Libraries).

### C# NuGet

Use the C# client libraries from NuGet.org. Currently, the package targets the netstandard2.0 and netcoreapp2.0 frameworks. For more information about the NuGet package, see [Microsoft.Dynamics.CustomerInsights.Api](https://www.nuget.org/packages/Microsoft.Dynamics.CustomerInsights.Api/).
...
1. Use the [Microsoft Authentication Library (MSAL)](/azure/active-directory/develop/msal-overview) to get an `AccessToken` by using your existing [Azure app registration](#create-a-new-app-registration-in-the-azure-portal).

1. After you authenticate and acquire a token, construct a new `HttpClient` or use an existing one. Set the **DefaultRequestHeaders "Authorization"** to **Bearer "access token"** and set **Ocp-Apim-Subscription-Key** to the [**subscription key** from your Customer Insights - Data environment](#get-started-trying-the-apis).

   Reset the **Authorization** header when appropriate. For example, when the token expires.

1. Pass this `HttpClient` into the construction of the `CustomerInsights` client.

   <!--   :::image type="content" source="media/httpclient-sample.png" alt-text="Sample of httpclient."::: -->

1. Make calls with the client to the "extension methods", for example, `GetAllInstancesAsync`. If you prefer access to the underlying `Microsoft.Rest.HttpOperationResponse`, use the "http message methods", for example, `GetAllInstancesWithHttpMessagesAsync`.
...
1. The response is likely `object` type because the method can return multiple types (for example, `IList<InstanceInfo>` and `ApiErrorResult`). To check the return type, use the objects in the response types specified on the [API details page](https://developer.ci.ai.dynamics.com/api-details#api=CustomerInsights) for that operation.
   ...
   If you need more information about the request, use the **http message methods** to access the raw response object.

### Node.js package

Use the Node.js client libraries available through NPM: https://www.npmjs.com/package/@microsoft/customerinsights
