...
- Delta tables exposed through **Fabric shortcuts** (including shortcuts to OneLake, ADLS Gen2, Amazon S3, and Google Cloud Storage).
...
Customer Insights - Data uses Delta version history to perform incremental refreshes. In a regular refresh, only the rows that changed since the last refresh are processed.

To avoid full-refresh failures caused by missing Delta versions:

- Maintain at least **15 days** of Delta version history on each ingested table.
- In your Fabric lakehouse, set both `delta.logRetentionDuration` and `delta.deletedFileRetentionDuration` to **15 days or more**.
