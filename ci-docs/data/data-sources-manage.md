...
Data sources can be refreshed on an automatic schedule or refreshed manually on demand, if you have the correct privileges. [On-premises data sources](connect-power-query.md#add-data-from-on-premises-data-sources) refresh on their own schedules, which are set up during data ingestion.
