...
description: Business account measures let you analyze key metrics from scratch. Learn how to build customer and business measures in Dynamics 365 Customer Insights - Data.
ms.date: 07/10/2026
...
Create measures at the level of individual accounts (customer measure) or at the level of all accounts (business measure).
...
1. Select **New**.
   ...
   :::image type="content" source="media/measure-b2b.png" alt-text="Screenshot of the empty configuration screen for a B2B measure.":::
...
1. To add filters, select the **Filter** in the configuration area. When you apply filters, only the records that match the filters are used to calculate the measure.
...
1. If there are values in your data that you must replace with an integer, select **Rules**. Configure the rule and make sure that you choose only whole numbers as replacements. For example, replace *null* with *0*.
...
1. If there are multiple paths between the data table you mapped and the *Customer* table, choose one of the identified [relationship paths](../relationships.md). Measure results can vary depending on the selected path.
   ...
   1. Select **Relationship path** and choose the path that should be used to identify your measure. If there's only a single path to the *Customer* table, this control doesn't show.
...
1. In the **Preview** area, you see the data schema of the measure output table, including filters and dimensions. The preview reacts dynamically to changes in the configuration.
