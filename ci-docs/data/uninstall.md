...
> [!IMPORTANT]
> Removing Customer Insights - Data is a permanent action. All data, configurations, segments, measures, exports, and predictions associated with the instance are deleted and can't be recovered.
...
| Configured data sources | Disconnected; source data in Azure Data Lake or Dataverse isn't deleted |
...
Perform these recommended steps before removal:

1. Export any data you need to retain (customer profiles, segments, measures).
1. Deactivate all active exports.
1. Notify users who depend on Customer Insights - Data outputs (for example, Power Automate flows, Power BI reports, and Dynamics 365 journeys).

## Remove Customer Insights - Data

1. Sign in to the [Power Platform admin center](https://admin.powerplatform.microsoft.com).
1. Select **Environments** and choose the environment from which you want to remove Customer Insights - Data.
1. Select **Resources** > **Dynamics 365 apps**.
1. Find **Dynamics 365 Customer Insights** in the list, select the **...** menu, and then select **Uninstall**.
1. Confirm the removal when prompted.
1. Wait for the uninstall process to complete. This can take several minutes.
...
1. Verify that Customer Insights - Data tables (prefixed with `msdynci_`) have been removed from Dataverse by going to **Power Apps** > **Tables**.
1. Confirm that the Customer Insights - Data application URL for the environment no longer loads.
