...
> [!TIP]
> This article provides an updated method to connect Customer Insights - Data to storage that's behind a firewall. Use the method in this article to connect to firewalled storage. Update existing connections that use [private links](private-link.md) by the end of 2025.
...
There are three scenarios where Customer Insights – Data needs to connect to a firewalled Azure Data Lake Storage account:
...
> [!IMPORTANT]
> Regardless of a storage account's network access setting, if a storage account has any private link connections, Customer Insights – Data also connects by using a private link. Azure treats the storage account as if public access is disabled.
...
- Runs a script to set up Power Platform to use the new private endpoints.

**Customer Insights – Data administrator**

- Creates data connections and selects “This storage account is behind a firewall.”
